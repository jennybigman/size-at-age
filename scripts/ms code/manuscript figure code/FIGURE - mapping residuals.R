# mapping residuals for main text
  ...
  # read mod function
	...
	pol_MV <- purrr::map2(MV_file_path, mod, read_mod_fun)
	...
	pol_MV_resids <- purrr::map2_dfr(pol_MV, "age-specific\nfields", resid_comp_fun)	
	pol_shared_resids <- purrr::map2_dfr(pol_shared, "shared spatial/\nspatiotemporal fields", resid_comp_fun)	
	pol_ssf_resids <- purrr::map2_dfr(pol_ssf, "shared spatial\nfield", resid_comp_fun)	
	...
	pol_resids$mod_name = factor(pol_resids$mod_name, 
															 levels = c("age-specific\nfields",
	...
	pol_resids_age1 <- pol_resids |>
		filter(age == 1 & year %in% years) 
	
	pol_resids_age12 <- pol_resids |>
		filter(age == 12 & year %in% years) 
	
	pol_resids_trim <- bind_rows(pol_resids_age1, pol_resids_age12)
	...
	p <- 
		...
		ggh4x::facet_nested(age_label + year ~ mod_name) +
	...
	####################################################
	# for all species and ages -- for Supplementary ####
	####################################################
