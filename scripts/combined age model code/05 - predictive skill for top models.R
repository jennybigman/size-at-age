# assess forcast skill of climate-population relationships (top models)
	...
	preds_fun <- function(mod, x){
		...
		nsims = 200 # increase to at least 200
		...
		preds_mean <- preds_df %>%
			rowwise() %>%
			summarise(mean_est = mean(c_across(1:ncol(preds)))) # no - take column mean, which is a mean across individuals for that draw, then take average and SE (sd) of those
		
		# but these are different years.....so how to subdivide
		preds_sd <- preds_df %>%
			rowwise() %>%
