# gams with smoothed age and temp effects 
  ...
  # 2. weight ~ age * temp
	pol_pretemp_int_age_bam <- bam(log_wt ~ age_f + s(presurvey_mean_temp, by = age_f) + 
																 	s(julian_day) + te(latitude, longitude) + 
																	s(ID_f, bs = "re") + # haul in year random effect
								 									s(haul_id_f, bs = "re") + # haul in year random effect
								 									s(cohort_f, bs = "re"),
  ...
  #				file = here("./output/model output/ACLIM temps/pol_pretemp_int_age_bam.rds"))
	...
	saveRDS(pol_pretemp_int_age_bam, 
	 				file = here("./output/model output PC/pol_pretemp_int_age_bam.rds"))
  ...
  # 2. weight ~ age * temp
	pcod_pretemp_int_age_bam <- bam(log_wt ~ age_f + s(presurvey_mean_temp, by = age_f) + 
															 	   s(julian_day) + te(latitude, longitude) + 
																   s(ID_f, bs = "re") + # haul in year random effect
								 								   s(haul_id_f, bs = "re") + # haul in year random effect
								 								   s(cohort_f, bs = "re"),
  ...
  saveRDS(pcod_pretemp_int_age_bam, 
  				file = here("./output/model output PC/pcod_pretemp_int_age_bam.rds"))
  ...
  # 2. weight ~ age * temp
	yfin_pretemp_int_age_bam <- bam(log_wt ~ age_f + s(presurvey_mean_temp, by = age_f) + 
																		s(julian_day) + te(latitude, longitude) + 
																		s(ID_f, bs = "re") + # haul in year random effect
								 										s(haul_id_f, bs = "re") + # haul in year random effect
								 										s(cohort_f, bs = "re"),
  ...
  saveRDS(yfin_pretemp_int_age_bam, 
  				file = here("./output/model output PC/yfin_pretemp_int_age_bam.rds"))
  ...
  #### year prior temps  ####
	...
	#1. weight ~ age + temp
	pol_yrtemp_age_bam <- bam(log_wt ~ age_f + s(mean_yr_temp) + 
															s(julian_day) + te(latitude, longitude) + 
															s(ID_f, bs = "re") + # haul in year random effect
								 							s(haul_id_f, bs = "re") + # haul in year random effect
								 							s(cohort_f, bs = "re"),
  ...
  # 2. weight ~ age * temp #### NEXT
	pol_yrtemp_int_age_bam <- bam(log_wt ~ age_f + s(mean_yr_temp, by = age_f) + 
																	s(julian_day) + te(latitude, longitude) + 
																	s(ID_f, bs = "re") + # haul in year random effect
								 									s(haul_id_f, bs = "re") + # haul in year random effect
								 									s(cohort_f, bs = "re"),
