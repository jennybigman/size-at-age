# model checks and comparisons
	...
	# file list for mods
	MV_file_path <- paste0(here(), "/output/model output/tinyVAST/fully MV/reml/")
	MV_file_list <- list.files(path = paste0(MV_file_path))
	
	shared_file_path <- paste0(here(), "/output/model output/tinyVAST/all shared/reml/")
	shared_file_list <- list.files(path = paste0(shared_file_path)) 
	
	ssf_file_path <- paste0(here(), "/output/model output/tinyVAST/spatial field only/reml/")
...
#
  ...
  #    ) |>
	...
	gt::gtsave(AIC_tab, file = paste0(here(), "/output/tables/for paper/", sp, "_AIC_MV_vs_shared_REML.png"))
