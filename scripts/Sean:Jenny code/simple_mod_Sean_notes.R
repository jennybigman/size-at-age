...
fit1 <-
  sdmTMB(
    #log_wt ~ 0 + age_f * poly(yrprior_btemp, 3, raw = TRUE), # add year
    log_wt ~ 0 + age_f * year_f + poly(yrprior_btemp, 3, raw = TRUE), 
    data = pcod_dat,
    mesh = mesh,
    spatial = "on",
    spatiotemporal = "iid",
...
fit_age
fit_age$sd_report #se on all est parameters in same parameter space as model
theta <- get_pars(fit_age)
2 * plogis(theta$rho_time_unscaled[1, 1]) - 1 #correlation coefficient on AR1 time varying process (would have another one if ST fields AR)
...
p <- get_pars(fit_age)
p$b_rw_t
p$b_rw_t[,,1] # rw values - plot against time colored by age (one for each age)
# are there diffrent time trends by age but model doesn't have that flexibility before, model would be using temperature effect to explain it
...
out <- furrr::future_map_dfr(all_ages, fit_one_age, .options = options)
...
out <- furrr::future_map_dfr(all_ages, fit_one_age, yrs_forecast = 2L, .options = options)
...
polys3 <- purrr::map_dfr(all_ages, \(a) { # anonymous function
...
out_no_temp <- furrr::future_map_dfr(all_ages, fit_one_age, yrs_forecast = 2L, 
	.formula = log_wt ~ 1, .options = options)
