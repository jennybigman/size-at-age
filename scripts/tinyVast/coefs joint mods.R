	...
	poly2_mods <- stringr::str_subset(poly2_mods, 'int')
	
	poly3_mods <- stringr::str_subset(file_list, 'poly3')
	poly3_mods <- stringr::str_subset(poly3_mods, 'int')

	lin_mods <- stringr::str_subset(file_list, 'lin')
	lin_mods <- stringr::str_subset(lin_mods, 'int')
  ...
  no_cov_mods <- lapply(no_cov_mod_paths, readRDS)
	...
	lin_mods <- lapply(lin_mod_paths, readRDS)
	 ...
	 nc_par_fun <- function(mod){
		...
		p <- as.list(summary(mod, 'fixed'))
		
		term <- "Intercept"
		estimate <- p$Estimate
		std.error <- p$Std_Error
		statistic <- p$z_value
		pvalue <- p$p_value
		
		ages <- as.factor(d$age_f)
		age <- levels(ages)
		age <- as.character(age)
		
		pars <- tibble(term, estimate, std.error, statistic, pvalue, sp, age)
	...
	saveRDS(no_cov_pars, file = paste0(output_path, "no_cov_pars_fullyMV_AR.rds"))
