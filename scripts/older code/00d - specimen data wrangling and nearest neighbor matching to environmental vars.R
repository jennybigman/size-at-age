#00d - data wrnagling specimen data and nearest neighbor matching
	...
	station_list[, c(4:7)] <- as.data.frame(RANN::nn2(roms_grid[, c('latitude', 'longitude')],
	...
	#station_list$roms_ID_1 <- pull(roms_grid[c(station_list$nn.idx.1), 'roms_ID'])
	
	#station_list$roms_ID_2 <- pull(roms_grid[c(station_list$nn.idx.2), 'roms_ID'])
	...
	# station 2536 has no data (problematic) so use 2537
	...
	station_list$roms_ID[station_list$roms_ID == 2536] <- 2537
  ...
  # any NAs?
  NAs <- nn_presurvey_vars[!complete.cases(nn_presurvey_vars), ]
  
	presurvey_vars <- left_join(nn_presurvey_vars, survey_grid_trim) %>% dplyr::select(-stationid)
  ...
  nn_yr_prior_vars <- lapply(1970:2023, yr_prior_func) %>% bind_rows()
...
#
	...
	fwrite(dat_all, file = here("./data/sp_dat_all_Apr2024.csv"))
