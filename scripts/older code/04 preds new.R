# 04 - predictions of weight and environmental variables (temp, oxy)
	...
	file_path_all <- "/output/model output/sdmTMB output/May 2024/poly models/"
	
	atooth_mod <- read_rds(file = paste0(here(), file_path_all, "presurvey_boxy_int_poly3atooth.rds"))
		 	
	new_dat <- expand_grid(
				presurvey_boxy = seq(
					from = min(atooth_dat$presurvey_boxy),
					to = max(atooth_dat$presurvey_boxy),
					length.out = 25),
				age_f = sort(unique(atooth_dat$age_f)),
