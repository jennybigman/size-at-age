# gams with sdmTMB 
  ...
  # pcod ####
	...
	sanity(presurvey_btemp_pcod_nj)
   ...
   # yellowfin sole ####
	...
	sanity(presurvey_btemp_yfin_nj)
	
	# check residuals - simulation-based residuals
	sims <- simulate(presurvey_btemp_yfin_nj, nsim = 250)
	dharma_sims <- sims %>% dharma_residuals(presurvey_btemp_yfin_nj)
	
	saveRDS(presurvey_btemp_yfin_nj, 
  				file = here("./output/model output/sdmTMB output/presurvey_btemp_yfin_nj.rds"))
 
	# model with factor-smooth interaction
	pc <- pc_matern(range_gt = 285, sigma_lt = 0.3) # set up prior

 	presurvey_btemp_int_yfin_nj <- sdmTMB(
			log_wt_std ~ age_f + s(presurvey_btemp, by = age_f),
			data = yfinsole_dat,
			mesh = yfin_mesh,
			spatial = "on",
 	...
 	sanity(presurvey_btemp_int_yfin_nj)
 	
 	sims <- simulate(presurvey_btemp_int_yfin_nj, nsim = 250)
	dharma_sims <- sims %>% dharma_residuals(presurvey_btemp_int_yfin_nj)
 	
  saveRDS(presurvey_btemp_int_yfin_nj, 
  				file = here("./output/model output/sdmTMB output/presurvey_btemp_int_yfin_nj.rds"))
  ...
  ####################################################
	...
	yrprior_btemp_pol_nj <- 
			sdmTMB(	
 					log_wt_std ~ age_f + s(yrprior_btemp),
					data = pollock_dat,
					mesh = pol_mesh,
					spatial = "on",
					time = "year",
	...
	sanity(yrprior_btemp_pol_nj)
	
	# check residuals - simulation-based residuals
	sims <- simulate(yrprior_btemp_pol_nj, nsim = 250)
	dharma_sims <- sims %>% dharma_residuals(yrprior_btemp_pol_nj)
	
	saveRDS(yrprior_btemp_pol_nj, 
  				file = here("./output/model output/sdmTMB output/yrprior_btemp_pol_nj.rds"))
 
 	# model with factor-smooth interaction	
 	yrprior_btemp_int_pol_nj <- sdmTMB(
			log_wt_std ~ age_f + s(yrprior_btemp, by = age_f),
			data = pollock_dat,
			mesh = pol_mesh,
			spatial = "on",
			time = "year",
 	...
 	sanity(yrprior_btemp_int_pol_nj) 
 	
 	sims <- simulate(yrprior_btemp_int_pol_nj, nsim = 250)
	dharma_sims <- sims %>% dharma_residuals(yrprior_btemp_int_pol_nj)
 	
  saveRDS(yrprior_btemp_int_pol_nj, 
  				file = here("./output/model output/sdmTMB output/yrprior_btemp_int_pol_nj.rds"))
 
  # pcod ####
  
 	yrprior_btemp_pcod_nj <- 
			sdmTMB(	
 				log_wt_std ~ age_f + s(yrprior_btemp),
				data = pcod_dat_trim,
				mesh = pcod_mesh_trim,
	...
	sanity(yrprior_btemp_pcod_nj)
	
	# check residuals - simulation-based residuals
	sims <- simulate(yrprior_btemp_pcod_nj, nsim = 250)
	dharma_sims <- sims %>% dharma_residuals(yrprior_btemp_pcod_nj)
	
	saveRDS(yrprior_btemp_pcod_nj, 
  				file = here("./output/model output/sdmTMB output/yrprior_btemp_pcod_nj.rds"))
 
	# model with factor-smooth interaction
	
	# set up prior
	pc <- pc_matern(range_gt = 235, sigma_lt = 0.4) 
		
 	yrprior_btemp_int_pcod_nj <- sdmTMB(
			log_wt_std ~ age_f + s(yrprior_btemp, by = age_f),
			data = pcod_dat_trim,
			mesh = pcod_mesh_trim,
			priors = sdmTMBpriors(matern_s = pc),
 	...
 	sanity(yrprior_btemp_int_pcod_nj) # ln_smooth_sigma SE too large?
 	
 	#### trouble converging ####
 	
 	sims <- simulate(yrprior_btemp_int_pcod_nj, nsim = 250)
	dharma_sims <- sims %>% dharma_residuals(yrprior_btemp_int_pcod_nj)
 	
  saveRDS(yrprior_btemp_int_pcod_nj, 
  				file = here("./output/model output/sdmTMB output/yrprior_btemp_int_pcod_nj.rds"))

  #### yellowfin sole ####
  
	yrprior_btemp_yfin_nj <- 
			sdmTMB(	
 				log_wt_std ~ age_f + s(yrprior_btemp),
				data = yfinsole_dat,
				mesh = yfin_mesh,
	...
	sanity(yrprior_btemp_yfin_nj)
	
	# check residuals - simulation-based residuals
	sims <- simulate(yrprior_btemp_yfin_nj, nsim = 250)
	dharma_sims <- sims %>% dharma_residuals(yrprior_btemp_yfin_nj)
	
	saveRDS(yrprior_btemp_yfin_nj, 
  				file = here("./output/model output/sdmTMB output/yrprior_btemp_yfin_nj.rds"))
 
	# adjust mesh 
	yfin_mesh2 <- make_mesh(yfinsole_dat, c("X", "Y"), n_knots = 800, type = "kmeans")

	# set up priors
	pc <- pc_matern(range_gt = 470, sigma_lt = 0.5) 
	
	yrprior_btemp_int_yfin_nj <- sdmTMB(
			log_wt_std ~ age_f + s(yrprior_btemp, by = age_f),
			data = yfinsole_dat,
			mesh = yfin_mesh,
			priors = sdmTMBpriors(matern_s = pc),
 	...
 	sanity(yrprior_btemp_int_yfin_nj)
 	
 	sims <- simulate(yrprior_btemp_int_yfin_nj, nsim = 250)
	dharma_sims <- sims %>% dharma_residuals(yrprior_btemp_int_yfin_nj)
 	
  saveRDS(yrprior_btemp_int_yfin_nj, 
  				file = here("./output/model output/sdmTMB output/presurvey_btemp_int_yfin_nj.rds"))
  ...
  #### pollock ####
	...
	sanity(age0_btemp_pol_nj)
	
	# check residuals - simulation-based residuals
	sims <- simulate(age0_btemp_pol_nj, nsim = 250)
	dharma_sims <- sims %>% dharma_residuals(age0_btemp_pol_nj)
	
	saveRDS(age0_btemp_pol_nj, 
  				file = here("./output/model output/sdmTMB output/age0_btemp_pol_nj.rds"))
 
 	# model with factor-smooth interaction	
 	age0_btemp_int_pol_nj <- sdmTMB(
 	...
 	sanity(age0_btemp_int_pol_nj) 
 	
 	sims <- simulate(age0_btemp_int_pol_nj, nsim = 250)
	dharma_sims <- sims %>% dharma_residuals(age0_btemp_int_pol_nj)
 	
  saveRDS(age0_btemp_int_pol_nj, 
  				file = here("./output/model output/sdmTMB output/age0_btemp_int_pol_nj.rds"))
  ...
  # adjust mesh due to errors
	...
	sanity(age0_btemp_pcod_nj)
	
	# check residuals - simulation-based residuals
	sims <- simulate(age0_btemp_pcod_nj, nsim = 250)
	dharma_sims <- sims %>% dharma_residuals(age0_btemp_pcod_nj)
	
	saveRDS(age0_btemp_pcod_nj, 
  				file = here("./output/model output/sdmTMB output/age0_btemp_pcod_nj.rds"))
 
	# model with factor-smooth interaction
	
	# set up priors
	pc_s <- pc_matern(range_gt = 260, sigma_lt = 0.4) 
	pc_st <- pc_matern(range_gt = 260, sigma_lt = 0.4)
	
	# adjust mesh
	pcod_mesh_trim2 <- make_mesh(pcod_dat_trim, xy_cols = c("X", "Y"), n_knots = 700)

 	age0_btemp_int_pcod_nj <- sdmTMB(
			log_wt_std ~ age_f + s(age0_btemp, by = age_f),
			data = pcod_dat_trim,
			mesh = pcod_mesh_trim2,
 	...
 	sims <- simulate(age0_btemp_int_pcod_nj, nsim = 250)
	dharma_sims <- sims %>% dharma_residuals(age0_btemp_int_pcod_nj)
 	
  saveRDS(age0_btemp_int_pcod_nj, 
  				file = here("./output/model output/sdmTMB output/age0_btemp_int_pcod_nj.rds"))
