# sdmTMB function - without CV, knots specified
	...
	file_path_all <- "/output/model output/sdmTMB output/April 2024/"
...
#################################
	...
	presurvey_btemp_no_int_pcod <-
		sdmTMB(
			formula = log_wt ~ 0 + age_f + s(presurvey_btemp, k = 3),
			data = pcod_dat,
			mesh = pcod_mesh,
			spatial = "on",
			spatiotemporal = "iid",
	...
	sanity(presurvey_btemp_no_int_pcod)

	write_rds(yrprior_btemp_no_int_pcod, 
						file = paste0(here(), 
						file_path_all, "yrprior_btemp_no_int_pcod.rds"))
