# sdmTMB function - no CV for now
	...
	file_path <- "/output/model output/sdmTMB output/with year as RE/"
	...
	test <- function(sp, y){
		...
		# wrangling and making a mesh 
  				...
  				# make year a factor and drop unused levels
					...
					# mod name for file path
					mod_name <- "_int_mod_yr_RE_"
		...
		# run models	
				...
				# deal with warnings and issues
					...
					# if no error and sanity() checks all good, save model and tell me
		 						...
		 						write_rds(mod_int, 
									file = paste0(here(), file_path, y, mod_name, sp, ".rds"))
		 		 ...
		 		 # if no error and sanity() does not look good aside from a few non-issues (see above), rerun model with extra optimization
 													 ...
 													 if (any(ind_eo %!in% not_probs)) { 
															...
															print('boo')
 													 		...
 													 		write_rds(mod_int_eo, 
 													 							file = paste0(here(), 
 													 							file_path, y, "_int_mod_yr_RE_", sp, ".rds"))
								...
								# if model with extra optimization (eo) looks all good, save model
		 									...
		 									write_rds(mod_int_eo, 
												file = paste0(here(), file_path, y, "_int_mod_yr_RE_", sp, ".rds"))
  ...
  lapply(pol_mod_list, AIC)
	...
	test <- read_rds(paste0(here(), file_path, "age0_boxy_int_mod_yr_RE_pollock.rds"))
	
	test2 <- run_extra_optimization(test)
	s_test <- sanity(test2)
	
	s_test$se_magnitude_ok == TRUE
	
	# looks like sanity() is the same?
	
	list_probs <- c(s_test$hessian_ok, s_test$eigen_values_ok, s_test$nlminb_ok, 
									s_test$range_ok, s_test$se_na_ok, s_test$sigmas_ok)
