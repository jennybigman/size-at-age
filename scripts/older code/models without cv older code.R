...
# sdmTMB function - no CV for now
	...
	sdmTMB_yr_RE_func <- function(sp, y){
		...
		# deal with warnings and issues
					...
					# if no error and sanity() checks all good, save model and tell me
		 						...
		 						write_rds(mod, 
									file = paste0(here(), file_path_all, y, "_no_int_mod_yr_RE_", sp, ".rds"))
											...
											ind_eo <- which(s_eo == FALSE) %>% as.numeric()
 													 ...
 													 if (any(ind_eo %!in% not_probs)) { 
															...
															print('boo')
 													 		...
 													 		write_rds(mod_eo, 
 													 							file = paste0(here(), 
 													 							file_path_all, y, "_no_int_mod_yr_RE_", sp, ".rds"))
  ...
  mods <- purrr::map(mod_names_list, readRDS)
  ...
  # separate models by species ####
	...
	pol_mod_list <- mods[grep("pol", names(mods))]
	
	# pcod #
	pcod_mod_list <- mods[grep("pcod", names(mods))]
		
	# yfin #
	yfin_mod_list <- mods[grep("yf", names(mods))]
