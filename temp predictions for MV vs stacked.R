# just read the rds file
	...
	# set grid cell for prediction
	X_pred <- min(pcod_dat$X)
	Y_pred <- min(pcod_dat$Y)
	
	# set temp range for prediction
	min_temp <- min(pcod_dat$yrprior_btemp, na.rm = TRUE)
	max_temp <- max(pcod_dat$yrprior_btemp, na.rm = TRUE)
